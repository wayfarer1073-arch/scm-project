...
- **잉크 (Ink)** (`oklch(0.19 0.018 260)`): 본문 텍스트, 헤드라인, 차트 라인(추세는 위험이 아니므로
  중립 잉크를 쓴다), **그리고 버튼 배경(`--primary`)**. 브랜드색(과거 cyan)을 걷어내고 버튼·활성
  탭·체크된 체크박스/스위치까지 전부 이 잉크 톤 하나로 통일한다.
- **잉크 뮤트 (Ink Muted)** (`oklch(0.5 0.02 260)`): 보조 텍스트, 라벨, 캡션.
- **페이퍼 (Paper)** (`oklch(1 0 0)`): 패널 배경.
- **보더 (Border)** (`oklch(0.9 0.009 255)`): 패널 테두리와 헤어라인 구분선.
- **호버 배경 (Accent)** (`oklch(0.93 0.005 260)`): outline/ghost 버튼과 select/dropdown 항목의
  hover 배경. 과거에는 옅은 cyan(`oklch(0.95 0.02 221)`)이었고, 흰 배경과 이질적인 색으로 보인다는
  피드백을 받아 잉크 계열 무채색으로 바꿨다.
...
### Button (공유 프리미티브, 전체 화면 공통)
- **Shape:** `rounded-md`, 그림자 없음(과거 `shadow-sm`을 모든 variant에서 제거).
- **default:** `bg-primary text-primary-foreground` — `--primary`가 잉크 톤이라 실질적으로
  `bg-foreground text-background`와 같다. Hover는 `/85` 불투명도로 살짝 어둡게(그림자가 없어진
  만큼 색 전환만으로 눌림이 느껴지도록 90→85로 조정).
- **outline / ghost:** hover 배경이 `--accent`(잉크 계열 무채색, 옅은 회색)로 바뀐다 — 과거 옅은
  cyan이 흰 배경과 이질적으로 보이던 문제를 해결.
- **destructive:** 삭제 등 실제로 되돌릴 수 없는 동작 전용. 위험 레드와는 별개 토큰(`--destructive`)
  이며 재고 위험 신호와 의미를 섞지 않는다.
- **secondary/link:** 각각 `--secondary`(중립 회색), `text-primary`(잉크)로 자연히 통일된다.
- 이 변경은 `--primary`/`--accent`/`--ring` 전역 토큰 수정만으로 이뤄졌다 — 버튼 컴포넌트뿐 아니라
  체크된 체크박스/스위치, 활성 탭, 포커스 링까지 한 번에 잉크 톤으로 맞춰진다.

### 토글/탭 버튼 (날짜 조회, 태그 선택 등)
- 활성 상태는 `bg-foreground text-background`(잉크 반전), 비활성은 `text-muted-foreground`.
  `shadow-sm`을 쓰던 이전 스타일은 전부 제거했다 — 상태 표현은 색 반전만으로 충분하다. `Button`의
  `default` variant와 사실상 같은 색 언어를 쓴다.
...
- **Do** 버튼·활성 탭·체크된 토글은 잉크 톤(`--primary`)을 쓴다. 색은 위험 신호에만 예약한다.
