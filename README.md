...
최신 업로드 목록에서 사라진 SKU는 운영 정책에 따라 **품절**로 분류합니다.
1개월간 이력 확인용으로 남기고 현재 재고금액·보유율
집계에서 제외합니다. 목록과 엑셀의 현재 수량·금액은 0으로 표시하며 상세에는 마지막 관측값을 보존합니다.
목록 이탈을 가상의 출고 이력으로 만들지는 않습니다. 다시 나타나면 새 관측 이력으로 분석합니다.
