...
- Card 박스를 버리고 패널 내부 `divide-x`/`divide-y` 그리드의 한 칸으로 존재한다(제목 + 헤어라인
  없이 바로 그래프).
- 도넛: 리더라인 라벨(카테고리별 고정 슬롯), 중앙에 전체 합계 숫자.
...
**범위 안내.** 이 문서는 메인 대시보드(`/`)에 한해 확정된 시스템이다. 업로드·설정·게시판·SKU 상세·
로그인 화면은 아직 이전 카드 기반 시스템을 그대로 쓰고 있으며, 이번 작업 범위 밖이었다(사용자가
"메인 대시보드만 먼저"를 선택함). 나머지 화면을 이 시스템으로 옮길 때 이 문서를 갱신한다.
