...
export function OperatingSummary({ rows }: { rows: InventoryRow[] }) {
  ...
  return <section className="rounded-xl border border-border p-5" aria-label="운영 유형별 재고">
    ...
    <div className="mt-3 grid gap-4 sm:grid-cols-3">
      ...
      <div><p className="text-sm font-medium">최근 목록 미관측 · {rows.filter(r => r.descriptor.isSoldOut).length} SKU</p><p className="text-xs text-muted-foreground">품절·판매 종료·일시 누락 확인 필요. 마지막 재고는 현재 자산에서 제외</p></div>
    </div>
