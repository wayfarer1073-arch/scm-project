...
import { shiftDate } from './shipping-calendar';
 ...
 *
 * 등록일이 실제 재고 증가가 관측된 날짜와 하루 어긋나 있어도(입력 착오) 반영되도록, 아래 두
 * 경우에 한해 ±1일 관용을 둔다. 둘 다 "원래 자리에서는 이 입고가 어차피 아무것도 설명하지
 * 못했을 때"만 옮긴다 — 원래 자리에 실제로 필요했다면(그 구간도 재고가 늘었다면) 그대로 둔다.
 * 그래서 이 관용은 기존에 올바르게 붙던 입고를 절대 빼앗지 않는다.
 *  - 마지막 관측일 다음날: 아직 그 날짜의 스냅샷이 없어 어느 구간에도 못 속하던 입고를
 *    마지막 관측치에 반영한다("업로드는 아직이지만 실제로는 이미 반영된 입고"로 취급).
 *  - 어느 관측일과 정확히 같은 날짜로 등록됐는데, 그 관측일로 끝나는 구간은 재고가 늘지
 *    않았고(그 등록이 거기선 아무 효과가 없었던 것) 바로 다음 관측이 정확히 하루 뒤이며
...
export function attachIntervalInbounds(observations: StockObservation[], entries: DatedInbound[]): StockObservation[] {
  ...
  // 경계일 공유 재배정: interval i가 필요 없었고, 바로 다음(정확히 하루 뒤) 구간이 필요로 하면 옮긴다.
  for (let i = 1; i < n - 1; i++) {
    if (perIntervalEntries[i].length === 0) continue;
    if (rawChange(i) > 0) continue;
    if (observations[i + 1].date !== shiftDate(observations[i].date, 1)) continue;
  ...
  // 마지막 관측일 다음날 등록분(하루 늦은 등록)은 마지막 관측치에 반영한다.
  if (leftover.length > 0) {
    const graceDate = shiftDate(observations[n - 1].date, 1);
    const graced = leftover.filter((e) => e.date === graceDate);
